...
import {Component} from 'gooy-angular/component';
...
export class CodeComponent extends Component {
  ...
  //static delegate() { return [EventAggregator]; }

  static rawInject() { return ['$compile', '$parse']; }

  attached() {
    this.model = this.ctrl;
    delete this.ctrl;

    this.language = CodeComponent.getLanguageFromClassname(this.element);
    ...
    this.$transclude((clone,innerScope)=>{this.transclude(clone,innerScope);});
  }
  ...
  transclude(clone) {
    ...
    if (this.model && this.model.$modelValue) {
      this.markReady();
      return;
    }
      ...
      switch (clone[i].nodeType) {
        case 1: // Node.ELEMENT_NODE
          if (clone[i] instanceof HTMLTemplateElement) { // jshint ignore:line
            ...
            //if(this.language === "html" || this.language === "markup"){
            //  data += clone[i].innerText.trim();
            //}else{
            data += clone[i].innerText;
            //}
  ...
  set value(code) {
    ...
    //this.rawValue = code;
    if(this.type==="dom"){
      this.code.innerHTML = CodeComponent.escapeHtml(code);
    }else {
    ...
    if (this.attr.template !== undefined && this.attr.template !== false) {
      ...
      this.rawValue = this.code.innerHTML;
    }
